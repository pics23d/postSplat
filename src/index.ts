...
console.log(`postSplat v${appVersion} | SplatTransform v${stVersion} (${stRevision}) | Engine v${engineVersion} (${engineRevision}) | PCUI v${pcuiVersion} (${pcuiRevision})`);
...
main().catch((err) => {
    ...
    const [key, fallback] = window.navigator.gpu ?
        ['startup.failed', 'postSplat failed to start. See the browser console for details.'] :
        ['startup.webgpu-unavailable', 'postSplat requires WebGPU, which this browser does not support. Please try the latest Chrome, Edge or Safari.'];
